...
```bash
sam-rds-example$ sam logs -n StockCheckerFunction --stack-name sam-rds-example --tail
```
...
```bash
sam-rds-example$ dotnet test tests/StockChecker.Test
```
...
```bash
aws cloudformation delete-stack --stack-name sam-rds-example
```
